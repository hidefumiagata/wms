...
vi.mock('@/api/generated/models/user-detail', () => ({}))
vi.mock('@/api/generated/models/user-page-response', () => ({}))
...
describe('useUserList', () => {
  ...
  it('onUnmounted 時に進行中のリクエストがキャンセルされる', async () => {
    const { result, wrapper } = withSetup(() => useUserList())
  ...
  it('handleReset が検索条件をクリアしてfetchListを呼ぶ', async () => {
    ...
    result.searchForm.keyword = 'テスト'
    result.searchForm.role = 'ADMIN'
    result.searchForm.status = 'ACTIVE'
    result.page.value = 5

    result.handleReset()
    await flushPromises()

    expect(result.searchForm.keyword).toBe('')
    expect(result.searchForm.role).toBe('')
    expect(result.searchForm.status).toBe('')
    expect(result.page.value).toBe(1)
  })
  ...
  it('サーバーエラー時にfetchErrorメッセージが表示される', async () => {
    ...
    expect(ElMessage.error).toHaveBeenCalledWith('master.user.fetchError')
  })

  // --- handleUnlock ---

  it('handleUnlock が確認後にPATCH APIを呼ぶ', async () => {
    ...
    expect(apiClient.patch).toHaveBeenCalledWith('/master/users/1/unlock')
    expect(ElMessage.success).toHaveBeenCalled()
  })

  it('handleUnlock で確認キャンセル時はAPIを呼ばない', async () => {
  ...
  it('handleUnlock のネットワークエラー時にエラーメッセージが表示される', async () => {
  ...
  it('handleUnlock のサーバーエラー時にunlockErrorメッセージが表示される', async () => {
    ...
    expect(ElMessage.error).toHaveBeenCalledWith('master.user.unlockError')
  })
})
