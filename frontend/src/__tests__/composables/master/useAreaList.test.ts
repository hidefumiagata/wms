...
vi.mock('@/api/generated/models/area-detail', () => ({}))
...
describe('useAreaList', () => {
  ...
  it('warehouseId が未設定の場合 fetchList は API を呼ばない', async () => {
    const { result } = withSetup(() => useAreaList())
  ...
  it('fetchBuildings が建物オプションを取得する', async () => {
    vi.mocked(apiClient.get).mockResolvedValueOnce(mockAxiosResponse(createMockBuildingsResponse()))
  ...
  it('fetchBuildings が warehouseId 未設定の場合何もしない', async () => {
    const { result } = withSetup(() => useAreaList())
  ...
  it('fetchBuildings がエラー時に空配列を設定する', async () => {
    ...
    expect(result.buildings.value).toHaveLength(0)
  })
  ...
  it('handleReset が検索条件をクリアしてfetchListを呼ぶ', async () => {
    ...
    result.searchForm.buildingId = 1
    result.searchForm.storageCondition = 'COLD'
    result.searchForm.areaType = 'RECEIVING'
    ...
    expect(result.searchForm.buildingId).toBeNull()
    expect(result.searchForm.storageCondition).toBe('')
    expect(result.searchForm.areaType).toBe('')
  ...
  it('サーバーエラー時にfetchErrorメッセージが表示される', async () => {
    ...
    expect(ElMessage.error).toHaveBeenCalledWith('master.area.fetchError')
  })
  ...
  it('handleToggleActive の422エラーでエラーメッセージが表示される', async () => {
    ...
    expect(ElMessage.error).toHaveBeenCalledWith('master.area.cannotDeactivateHasChildren')
  })
})
