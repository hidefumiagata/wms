export default {
  common: {
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    search: 'Search',
    reset: 'Reset',
    clear: 'Clear',
    close: 'Close',
    confirm: 'Confirm',
    back: 'Back',
    yes: 'Yes',
    no: 'No',
    loading: 'Loading...',
    noData: 'No data',
    required: 'Required',
    active: 'Active',
    inactive: 'Inactive',
    actions: 'Actions',
    total: '{total} records total',
  },
  auth: {
    login: 'Login',
    logout: 'Logout',
    userCode: 'User Code',
    password: 'Password',
    currentPassword: 'Current Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm New Password',
    identifier: 'User Code / Email Address',
    changePassword: 'Initial Password Change',
    resetPassword: 'Password Reset',
    resetConfirm: 'Reset Password',
    sendResetLink: 'Send Reset Link',
    loginButton: 'Login',
    changePasswordButton: 'Change Password',
    resetPasswordButton: 'Reset Password',
    forgotPassword: 'Forgot your password?',
    backToLogin: 'Back to Login',
    logoutLink: 'Logout from this account',
    logoutConfirm: 'Are you sure you want to logout?',
    sessionWarningTitle: 'Session Timeout Warning',
    sessionWarning:
      'You have been inactive for 55 minutes. You will be automatically logged out in 5 minutes. Would you like to extend your session?',
    continueSession: 'Extend Session',
    versionInfo:
      'WMS v1.0 — Please contact your system administrator if you have trouble logging in',
    passwordChangeNotice:
      'This is your first login. Please change your password. You cannot access other features until you do.',
    passwordPolicyInfo:
      '8–128 characters, including at least one uppercase letter, one lowercase letter, and one digit',
    resetRequestDescription: 'Enter your registered user code or email address',
    strength: {
      weak: 'Weak',
      medium: 'Medium',
      strong: 'Strong',
    },
    messages: {
      sessionExpired: 'Your session has expired. Please log in again.',
      invalidCredentials: 'Invalid user code or password',
      accountLocked: 'Account is locked. Please contact your administrator.',
      loginFailed500: 'Login failed. Please try again later.',
      passwordChanged: 'Password changed successfully. Welcome to WMS.',
      currentPasswordWrong: 'Current password is incorrect',
      passwordChangeFailed500: 'Failed to change password. Please try again later.',
      resetRequestSent: 'Email sent. If you do not receive it, please contact your administrator.',
      resetRequestFailed: 'Failed to send request. Please try again later.',
      resetTokenInvalid: 'The reset link is invalid or has expired. Please request a new one.',
      resetConfirmFailed: 'Failed to reset password. Please try again later.',
      passwordResetSuccess: 'Password reset successfully. Please log in with your new password.',
      tokenMissing: 'Invalid reset link. Please start over from the password reset request.',
      rateLimitExceeded: 'Too many requests. Please wait a moment and try again.',
    },
    validation: {
      userCodeRequired: 'User code is required',
      passwordRequired: 'Password is required',
      currentPasswordRequired: 'Current password is required',
      newPasswordRequired: 'New password is required',
      passwordMinLength: 'Password must be at least 8 characters',
      passwordSameAsCurrent: 'New password must be different from the current password',
      confirmPasswordRequired: 'Password confirmation is required',
      identifierRequired: 'Please enter your user code or email address',
      passwordPolicy:
        'Password must be 8–128 characters and include at least one uppercase letter, one lowercase letter, and one digit',
    },
  },
  nav: {
    dashboard: 'Dashboard',
    master: 'Master Data',
    warehouse: 'Warehouse',
    product: 'Product',
    partner: 'Partner',
    user: 'User',
    inbound: 'Inbound',
    inboundSlips: 'Inbound Slips',
    inboundResults: 'Inbound Results',
    inventory: 'Inventory',
    inventoryList: 'Inventory List',
    inventoryMove: 'Move',
    inventoryBreakdown: 'Breakdown',
    inventoryCorrection: 'Correction',
    inventoryStocktake: 'Stocktake',
    outbound: 'Outbound',
    outboundSlips: 'Orders',
    pickingInstructions: 'Picking Instructions',
    returns: 'Returns',
    returnsNew: 'Return Registration',
    allocation: 'Allocation',
    batch: 'Batch',
    batchHistory: 'Execution History',
    interfaceManagement: 'Interface',
    interfaceFiles: 'File List',
    interfaceHistory: 'Import History',
    system: 'System',
    systemParameters: 'Parameters',
    building: 'Building',
    area: 'Area',
    location: 'Location',
    selectWarehouse: 'Select warehouse',
  },
  master: {
    partner: {
      title: 'Partner Master',
      list: 'Partner List',
      create: 'New Partner',
      edit: 'Edit Partner',
      sectionBasicInfo: 'Basic Information',
      sectionContactInfo: 'Contact Information',
      partnerCode: 'Partner Code',
      partnerName: 'Partner Name',
      partnerNameKana: 'Partner Name (Kana)',
      partnerType: 'Type',
      address: 'Address',
      phone: 'Phone',
      contactPerson: 'Contact Person',
      email: 'Email',
      status: 'Status',
      statusAll: 'All',
      typeAll: 'All',
      typeSupplier: 'Supplier',
      typeCustomer: 'Customer',
      typeBoth: 'Both',
      newRegistration: 'New',
      register: 'Register',
      activate: 'Activate',
      deactivate: 'Deactivate',
      codePlaceholder: 'e.g. P-001',
      kanaPlaceholder: 'e.g. カブシキガイシャエービーシーショウジ',
      addressPlaceholder: 'e.g. 1-1-1 Tatsumi, Koto-ku, Tokyo',
      phonePlaceholder: 'e.g. 03-1234-5678',
      contactPersonPlaceholder: 'e.g. Taro Yamada',
      emailPlaceholder: 'e.g. contact@example.com',
      createSuccess: 'Partner registered successfully.',
      updateSuccess: 'Partner updated successfully.',
      activateSuccess: 'Partner activated.',
      deactivateSuccess: 'Partner deactivated.',
      fetchError: 'Failed to load data. Please try again later.',
      notFound: 'The specified partner does not exist or has been deleted.',
      cannotDeactivateHasActiveInbound: 'Cannot deactivate because there are active inbound plans.',
      cannotDeactivateHasActiveOutbound:
        'Cannot deactivate because there are active outbound orders.',
      confirmDeactivate:
        'Deactivate this partner? Deactivated partners cannot be selected for new inbound/outbound registrations.',
      confirmActivate: 'Activate this partner?',
      validation: {
        codeRequired: 'Partner code is required',
        codeFormat: 'Partner code must contain only alphanumeric characters and hyphens',
        codeMaxLength: 'Partner code must be 50 characters or less',
        codeDuplicate: 'This code is already registered',
        nameRequired: 'Partner name is required',
        nameMaxLength: 'Partner name must be 200 characters or less',
        kanaRequired: 'Partner name (Kana) is required',
        kanaMaxLength: 'Partner name (Kana) must be 200 characters or less',
        kanaFormat: 'Partner name (Kana) must be in full-width Katakana',
        addressMaxLength: 'Address must be 200 characters or less',
        phoneMaxLength: 'Phone must be 20 characters or less',
        phoneFormat: 'Phone must contain only digits and hyphens',
        contactPersonMaxLength: 'Contact person must be 50 characters or less',
        emailMaxLength: 'Email must be 254 characters or less',
        emailFormat: 'Please enter a valid email address',
      },
    },
    product: {
      title: 'Product Master',
      list: 'Product List',
      create: 'New Product',
      edit: 'Edit Product',
      sectionBasicInfo: 'Basic Information',
      sectionStorageManagement: 'Storage & Management',
      sectionFlags: 'Flag Settings',
      productCode: 'Product Code',
      productName: 'Product Name',
      productNameKana: 'Product Name (Kana)',
      barcode: 'Barcode / JAN Code',
      storageCondition: 'Storage Condition',
      storageConditionAll: 'All',
      storageAmbient: 'Ambient',
      storageRefrigerated: 'Refrigerated',
      storageFrozen: 'Frozen',
      caseQuantity: 'Case Quantity',
      ballQuantity: 'Ball Quantity',
      isHazardous: 'Hazardous Material',
      lotManageFlag: 'Lot Management',
      expiryManageFlag: 'Expiry Date Management',
      shipmentStopFlag: 'Shipment Stop',
      isActive: 'Active / Inactive',
      status: 'Status',
      statusAll: 'All',
      newRegistration: 'New',
      register: 'Register',
      activate: 'Activate',
      deactivate: 'Deactivate',
      codePlaceholder: 'e.g. PRD-001',
      kanaPlaceholder: 'e.g. ショウヒン',
      barcodePlaceholder: 'e.g. 4901234567890',
      createSuccess: 'Product registered successfully.',
      updateSuccess: 'Product updated successfully.',
      activateSuccess: 'Product activated.',
      deactivateSuccess: 'Product deactivated.',
      fetchError: 'Failed to load data. Please try again later.',
      notFound: 'The specified product does not exist or has been deleted.',
      cannotDeactivateHasInventory: 'Cannot deactivate because inventory exists.',
      confirmDeactivate:
        'Deactivate this product? Deactivated products cannot be selected for new inbound/outbound/stocktake registrations.',
      confirmActivate: 'Activate this product?',
      confirmCancel: 'Your changes will be lost. Are you sure you want to cancel?',
      cannotChangeLotFlag: 'Cannot change lot management flag because inventory exists.',
      cannotChangeExpiryFlag: 'Cannot change expiry management flag because inventory exists.',
      inventoryExistsTooltip: 'Cannot change because inventory exists',
      validation: {
        codeRequired: 'Product code is required',
        codeFormat: 'Product code must contain only alphanumeric characters and hyphens',
        codeMaxLength: 'Product code must be 20 characters or less',
        codeDuplicate: 'This code is already registered',
        nameRequired: 'Product name is required',
        nameMaxLength: 'Product name must be 200 characters or less',
        kanaRequired: 'Product name (Kana) is required',
        kanaMaxLength: 'Product name (Kana) must be 200 characters or less',
        kanaFormat: 'Product name (Kana) must be in full-width Katakana',
        barcodeMaxLength: 'Barcode must be 20 characters or less',
        barcodeFormat: 'Barcode must contain only digits',
        caseQuantityRange: 'Case quantity must be between 1 and 9999',
        ballQuantityRange: 'Ball quantity must be between 1 and 9999',
      },
    },
    warehouse: {
      title: 'Warehouse Master',
      list: 'Warehouse List',
      create: 'New Warehouse',
      edit: 'Edit Warehouse',
      warehouseCode: 'Warehouse Code',
      warehouseName: 'Warehouse Name',
      warehouseNameKana: 'Warehouse Name (Kana)',
      address: 'Address',
      status: 'Status',
      statusAll: 'All',
      newRegistration: 'New',
      register: 'Register',
      activate: 'Activate',
      deactivate: 'Deactivate',
      codePlaceholder: 'e.g. WARA',
      kanaPlaceholder: 'e.g. トウキョウディーシー',
      addressPlaceholder: 'e.g. 1-1-1 Tatsumi, Koto-ku, Tokyo',
      createSuccess: 'Warehouse registered successfully.',
      updateSuccess: 'Warehouse updated successfully.',
      activateSuccess: 'Warehouse activated.',
      deactivateSuccess: 'Warehouse deactivated.',
      fetchError: 'Failed to load data. Please try again later.',
      notFound: 'The specified warehouse does not exist or has been deleted.',
      cannotDeactivateHasInventory:
        'Cannot deactivate warehouse because it has existing inventory.',
      confirmDeactivate:
        'Deactivate this warehouse? Warehouses with existing inventory cannot be deactivated.',
      confirmActivate: 'Activate this warehouse?',
      validation: {
        codeRequired: 'Warehouse code is required',
        codeFormat: 'Warehouse code must be exactly 4 uppercase letters',
        codeDuplicate: 'This code is already registered',
        nameRequired: 'Warehouse name is required',
        nameMaxLength: 'Warehouse name must be 200 characters or less',
        kanaRequired: 'Warehouse name (Kana) is required',
        kanaMaxLength: 'Warehouse name (Kana) must be 200 characters or less',
        kanaFormat: 'Warehouse name (Kana) must be in full-width Katakana',
        addressMaxLength: 'Address must be 500 characters or less',
      },
    },
    building: {
      title: 'Building Master',
      list: 'Building List',
      create: 'New Building',
      edit: 'Edit Building',
      buildingCode: 'Building Code',
      buildingName: 'Building Name',
      warehouseCode: 'Warehouse Code',
      status: 'Status',
      statusAll: 'All',
      newRegistration: 'New',
      register: 'Register',
      activate: 'Activate',
      deactivate: 'Deactivate',
      codePlaceholder: 'e.g. A',
      namePlaceholder: 'e.g. Building A',
      codeHint: 'Building code cannot be changed after registration',
      createSuccess: 'Building registered successfully.',
      updateSuccess: 'Building updated successfully.',
      activateSuccess: 'Building activated.',
      deactivateSuccess: 'Building deactivated.',
      fetchError: 'Failed to load data. Please try again.',
      notFound: 'The specified building was not found.',
      cannotDeactivateHasChildren: 'Cannot deactivate because this building has child areas.',
      confirmDeactivate: 'Are you sure you want to deactivate this building?',
      confirmActivate: 'Activate this building?',
      confirmCancel: 'Your changes will be lost. Are you sure you want to cancel?',
      validation: {
        codeRequired: 'Building code is required',
        codeMaxLength: 'Building code must be 10 characters or less',
        codeFormat: 'Building code must contain only alphanumeric characters',
        codeDuplicate: 'This code is already registered',
        nameRequired: 'Building name is required',
        nameMaxLength: 'Building name must be 200 characters or less',
      },
    },
    area: {
      title: 'Area Master',
      list: 'Area List',
      create: 'New Area',
      edit: 'Edit Area',
      areaCode: 'Area Code',
      areaName: 'Area Name',
      buildingCode: 'Building Code',
      building: 'Building',
      buildingAll: 'All',
      warehouseCode: 'Warehouse Code',
      storageCondition: 'Storage Condition',
      storageConditionAll: 'All',
      storageAmbient: 'Ambient',
      storageRefrigerated: 'Refrigerated',
      storageFrozen: 'Frozen',
      areaType: 'Area Type',
      areaTypeAll: 'All',
      typeStock: 'Stock',
      typeInbound: 'Inbound',
      typeOutbound: 'Outbound',
      typeReturn: 'Return',
      status: 'Status',
      statusAll: 'All',
      newRegistration: 'New',
      register: 'Register',
      activate: 'Activate',
      deactivate: 'Deactivate',
      selectBuilding: 'Select building',
      codePlaceholder: 'e.g. A01',
      namePlaceholder: 'e.g. Building A Floor 1 Ambient Area',
      codeHint: 'Area code cannot be changed after registration',
      createSuccess: 'Area registered successfully.',
      updateSuccess: 'Area updated successfully.',
      activateSuccess: 'Area activated.',
      deactivateSuccess: 'Area deactivated.',
      fetchError: 'Failed to load data. Please try again.',
      notFound: 'The specified area was not found.',
      cannotDeactivateHasChildren: 'Cannot deactivate because this area has child locations.',
      confirmDeactivate: 'Are you sure you want to deactivate this area?',
      confirmActivate: 'Activate this area?',
      confirmCancel: 'Your changes will be lost. Are you sure you want to cancel?',
      validation: {
        buildingRequired: 'Building is required',
        codeRequired: 'Area code is required',
        codeMaxLength: 'Area code must be 20 characters or less',
        codeFormat: 'Area code must contain only alphanumeric characters and hyphens',
        codeDuplicate: 'This code is already registered in this building',
        nameRequired: 'Area name is required',
        nameMaxLength: 'Area name must be 200 characters or less',
        storageConditionRequired: 'Storage condition is required',
        areaTypeRequired: 'Area type is required',
      },
    },
    location: {
      title: 'Location Master',
      list: 'Location List',
      create: 'New Location',
      edit: 'Edit Location',
      locationCode: 'Location Code',
      locationName: 'Location Name',
      areaCode: 'Area Code',
      area: 'Area',
      areaAll: 'All',
      warehouseCode: 'Warehouse Code',
      codePrefix: 'Code (prefix match)',
      codePrefixPlaceholder: 'e.g. A-01',
      status: 'Status',
      statusAll: 'All',
      newRegistration: 'New',
      register: 'Register',
      activate: 'Activate',
      deactivate: 'Deactivate',
      selectArea: 'Select area',
      codePlaceholder: 'e.g. A-01-A-01-01-01',
      namePlaceholder: 'e.g. Shelf 1 Level 1 Column 1',
      codeHint: 'Location code cannot be changed after registration',
      codeFormatHintStock:
        'Stock area format: Building-Floor-Area-Shelf-Level-Column (e.g. A-01-A-01-01-01)',
      createSuccess: 'Location registered successfully.',
      updateSuccess: 'Location updated successfully.',
      activateSuccess: 'Location activated.',
      deactivateSuccess: 'Location deactivated.',
      fetchError: 'Failed to load data. Please try again.',
      notFound: 'The specified location was not found.',
      cannotDeactivateHasInventory: 'Cannot deactivate because this location has inventory.',
      cannotDeactivateStocktakeInProgress:
        'Cannot deactivate because this location is under stocktake.',
      areaLocationLimitExceeded:
        'This area already has a location registered (Inbound/Outbound/Return areas allow only one location).',
      confirmDeactivate: 'Are you sure you want to deactivate this location?',
      confirmActivate: 'Activate this location?',
      confirmCancel: 'Your changes will be lost. Are you sure you want to cancel?',
      validation: {
        areaRequired: 'Area is required',
        codeRequired: 'Location code is required',
        codeMaxLength: 'Location code must be 30 characters or less',
        codeFormatStock: 'Invalid location code format (e.g. A-01-A-01-01-01)',
        codeDuplicate: 'This code is already registered',
        nameMaxLength: 'Location name must be 200 characters or less',
      },
    },
    user: {
      title: 'User Master',
      list: 'User List',
      create: 'New User',
      edit: 'Edit User',
      sectionBasicInfo: 'Basic Information',
      sectionPassword: 'Initial Password',
      sectionRegistrationInfo: 'Registration Info (Read-only)',
      userCode: 'User Code',
      fullName: 'Full Name',
      email: 'Email',
      role: 'Role',
      roleAll: 'All',
      status: 'Status',
      statusAll: 'All',
      statusActive: 'Active',
      statusInactive: 'Inactive',
      statusLocked: 'Locked',
      isActive: 'Active / Inactive',
      keyword: 'Name / User Code',
      keywordPlaceholder: 'Search by name or user code',
      initialPassword: 'Initial Password',
      confirmPassword: 'Confirm Password',
      createdAt: 'Created At',
      updatedAt: 'Updated At',
      passwordChangeRequired: 'Password Change Required',
      passwordChangeNeeded: 'Change required (pending)',
      passwordChangeCompleted: 'Changed',
      newRegistration: 'New',
      register: 'Register',
      unlock: 'Unlock',
      selectRole: 'Select role',
      codePlaceholder: 'e.g. staff001',
      namePlaceholder: 'e.g. Hanako Suzuki',
      emailPlaceholder: 'e.g. suzuki@example.com',
      passwordPlaceholder: '8 characters or more',
      confirmPasswordPlaceholder: 'Re-enter password',
      passwordPolicyHint:
        '8 or more characters. The user will be required to change their password on first login.',
      selfRoleHint: 'You cannot change your own role',
      selfDeactivateHint: 'You cannot deactivate yourself',
      lockedAlert: 'This account is locked',
      createSuccess: 'User registered successfully.',
      updateSuccess: 'User updated successfully.',
      unlockSuccess: 'Account unlocked successfully.',
      unlockError: 'Failed to unlock account. Please try again.',
      fetchError: 'Failed to load data. Please try again later.',
      notFound: 'User not found. Returning to the list.',
      cannotChangeSelfRole: 'You cannot change your own role.',
      cannotDeactivateSelf: 'You cannot deactivate yourself.',
      confirmUnlock: 'Unlock account for {name} ({code})?',
      confirmCancel: 'Your changes will be lost. Are you sure you want to cancel?',
      validation: {
        codeRequired: 'User code is required',
        codeMaxLength: 'User code must be 50 characters or less',
        codeFormat: 'User code must contain only alphanumeric characters and underscores',
        codeDuplicate: '{code} is already registered',
        nameRequired: 'Full name is required',
        nameMaxLength: 'Full name must be 200 characters or less',
        emailRequired: 'Email is required',
        emailMaxLength: 'Email must be 200 characters or less',
        emailFormat: 'Please enter a valid email address',
        roleRequired: 'Role is required',
        passwordMinLength: 'Password must be at least 8 characters',
        passwordMaxLength: 'Password must be 128 characters or less',
        confirmPasswordRequired: 'Password confirmation is required',
        passwordMismatch: 'Passwords do not match',
      },
    },
  },
  system: {
    parameters: {
      title: 'System Parameters',
      paramName: 'Parameter',
      currentValue: 'Current Value',
      defaultValue: 'Default',
      description: 'Description',
      updatedAt: 'Updated At',
      categoryInventory: 'INVENTORY',
      categoryOutbound: 'OUTBOUND',
      categoryInbound: 'INBOUND',
      categorySystem: 'SYSTEM',
      categorySecurity: 'SECURITY',
      fetchError: 'Failed to load data. Please try again later.',
      saveSuccess: 'Parameter updated successfully.',
      saveError: 'Failed to update parameter. Please try again.',
      validationError: 'Invalid value. Please enter a valid value.',
      confirmSave: 'Change "{name}" to "{value}"?',
      validation: {
        required: 'Value is required',
        integerFormat: 'Please enter a positive integer',
        stringMaxLength: 'Must be 500 characters or less',
        booleanFormat: 'Please select true or false',
      },
    },
  },
  validation: {
    required: '{field} is required',
    maxLength: '{field} must be {max} characters or less',
    pattern: 'Invalid format for {field}',
    emailInvalid: 'Please enter a valid email address',
    passwordMismatch: 'Passwords do not match. Please re-enter.',
  },
  inbound: {
    slip: {
      title: 'Inbound Slip List',
      detail: 'Inbound Slip Detail',
      newRegistration: 'New Registration',
      slipNumber: 'Slip Number',
      slipType: 'Type',
      typeNormal: 'Normal',
      typeBatch: 'Batch',
      plannedDate: 'Planned Date',
      plannedDateFrom: 'Planned Date (From)',
      plannedDateTo: 'Planned Date (To)',
      partner: 'Supplier',
      partnerAll: 'All',
      status: 'Status',
      statusAll: 'All',
      statusPlanned: 'Planned',
      statusConfirmed: 'Confirmed',
      statusInspecting: 'Inspecting',
      statusPartialStored: 'Partial Stored',
      statusStored: 'Stored',
      statusCancelled: 'Cancelled',
      lineCount: 'Lines',
      createdAt: 'Created',
      note: 'Note',
      createdBy: 'Created By',
      basicInfo: 'Basic Information',
      lines: 'Inbound Lines',
      lineNo: 'No',
      productCode: 'Product Code',
      productName: 'Product Name',
      unitType: 'Unit Type',
      lotNumber: 'Lot Number',
      expiryDate: 'Expiry Date',
      plannedQty: 'Planned Qty',
      inspectedQty: 'Inspected Qty',
      differenceQty: 'Difference',
      backToList: 'Back to List',
      confirm: 'Confirm Receipt',
      cancel: 'Cancel',
      goInspect: 'Inspect',
      goStore: 'Store',
      confirmMessage: 'Confirm receipt. Are you sure?',
      cancelMessage: 'Cancel this inbound slip. This cannot be undone. Are you sure?',
      cancelPartialStoredMessage:
        'Some items have already been stored. Cancellation will rollback stored inventory. Are you sure?',
      confirmSuccess: 'Receipt confirmed',
      cancelSuccess: 'Inbound slip cancelled',
      fetchError: 'Failed to fetch inbound slips',
      notFound: 'Inbound slip not found',
      cancelForbidden: 'Cannot cancel: already stored',
      reportInboundPlan: 'Inbound Plan Report',
      reportUnreceivedRt: 'Unreceived List (RT)',
      reportDownloading: 'Generating report...',
      reportDownloadError: 'Failed to download report',
    },
  },
  outbound: {
    slip: {
      title: 'Order List',
      detail: 'Order Detail',
      newRegistration: 'New Order',
      slipNumber: 'Slip Number',
      slipType: 'Type',
      typeNormal: 'Normal',
      typeTransfer: 'Warehouse Transfer',
      plannedDate: 'Planned Ship Date',
      plannedDateFrom: 'Planned Date (From)',
      plannedDateTo: 'Planned Date (To)',
      partner: 'Customer',
      partnerAll: 'All',
      status: 'Status',
      statusAll: 'All',
      statusOrdered: 'Ordered',
      statusPartialAllocated: 'Partial Allocated',
      statusAllocated: 'Allocated',
      statusPickingCompleted: 'Picking Completed',
      statusInspecting: 'Inspecting',
      statusShipped: 'Shipped',
      statusCancelled: 'Cancelled',
      lineCount: 'Lines',
      createdAt: 'Created',
      note: 'Note',
      createdBy: 'Created By',
      basicInfo: 'Basic Information',
      lines: 'Order Lines',
      lineNo: 'No',
      productCode: 'Product Code',
      productName: 'Product Name',
      unitType: 'Unit Type',
      orderedQty: 'Ordered Qty',
      allocatedQty: 'Allocated Qty',
      pickingQty: 'Picking Qty',
      inspectedQty: 'Inspected Qty',
      shippedQty: 'Shipped Qty',
      backToList: 'Back to List',
      allocate: 'Allocate',
      bulkAllocate: 'Bulk Allocate',
      cancel: 'Cancel',
      goPickingNew: 'Create Picking',
      goInspect: 'Inspect',
      goShip: 'Ship',
      allocateMessage: 'Allocate inventory for {count} selected order(s). Are you sure?',
      allocateSingleMessage: 'Allocate inventory for this order. Are you sure?',
      cancelMessage: 'Cancel this order. Are you sure?',
      allocateSuccess: 'Allocation completed',
      allocatePartialError: 'Some lines have insufficient stock. Please check details',
      cancelSuccess: 'Order cancelled',
      fetchError: 'Failed to fetch orders',
      notFound: 'Order not found',
      selectRequired: 'Select at least one order to allocate',
      carrier: 'Carrier',
      trackingNumber: 'Tracking Number',
      shippedAt: 'Shipped At',
      shippedBy: 'Shipped By',
      cancelledAt: 'Cancelled At',
      cancelledBy: 'Cancelled By',
    },
  },
  inventory: {
    title: 'Inventory List',
    locationView: 'By Location',
    productSummaryView: 'Product Summary',
    locationCode: 'Location',
    productCode: 'Product Code',
    productName: 'Product Name',
    product: 'Product',
    unitType: 'Unit Type',
    unitTypeAll: 'All',
    unitTypeCase: 'Case',
    unitTypeBall: 'Ball',
    unitTypePiece: 'Piece',
    storageCondition: 'Storage Condition',
    storageConditionAll: 'All',
    storageAmbient: 'Ambient',
    storageRefrigerated: 'Refrigerated',
    storageFrozen: 'Frozen',
    quantity: 'Quantity',
    allocatedQty: 'Allocated',
    availableQty: 'Available',
    caseQuantity: 'Case Total',
    ballQuantity: 'Ball Total',
    pieceQuantity: 'Piece Total',
    totalPieceEquivalent: 'Total (Piece Equiv.)',
    totalAllocatedQty: 'Total Allocated',
    totalAvailableQty: 'Total Available',
    move: 'Move',
    fetchError: 'Failed to fetch inventory. Please try again later.',
    noResult: 'No inventory matching the search criteria.',
    inventoryReport: 'Inventory Report',
    transitionReport: 'Transition Report',
    // Validation (INV-002~004)
    validation: {
      locationCodeRequired: 'Please enter a location code.',
      productRequired: 'Please select a product.',
      unitTypeRequired: 'Please select a unit type.',
      toLocationCodeRequired: 'Please enter a destination location code.',
      moveQtyRequired: 'Please enter a move quantity.',
      breakdownQtyRequired: 'Please enter a breakdown quantity.',
      toUnitTypeRequired: 'Please select a target unit type.',
      newQtyRequired: 'Please enter a new quantity.',
      reasonRequired: 'Please enter a reason.',
      reasonMaxLength: 'Reason must be 200 characters or less.',
    },
    // INV-002
    moveTitle: 'Move Inventory',
    fromLocation: 'From',
    toLocation: 'To',
    fromLocationCode: 'From Location',
    toLocationCode: 'To Location',
    currentQty: 'Current Qty',
    moveQty: 'Move Qty',
    moveConfirmMessage: 'Move {qty} {unitType} from {from} to {to}?',
    moveSuccess: 'Inventory moved successfully.',
    moveError: 'Failed to move inventory. Please try again later.',
    moveQtyExceedsAvailable: 'Move quantity exceeds available stock.',
    capacityExceeded: 'Exceeds location capacity. (Max: {max}{pkg}, After move: {after}{pkg})',
    locationCapacity: 'Location Capacity',
    sameLocationError: 'From and To locations must be different.',
    locationNotFound: 'Location code not found.',
    stocktakeLocked: 'Location is locked for stocktake.',
    // INV-003
    breakdownTitle: 'Breakdown Registration',
    fromUnitType: 'From Unit Type',
    toUnitType: 'To Unit Type',
    breakdownQty: 'Breakdown Qty',
    conversionPreview: 'Conversion Preview',
    convertedQty: 'Converted Qty',
    conversionRate: 'Conversion Rate',
    toLocationOptional: 'To Location',
    toLocationSameHint: '(Leave empty to use same location)',
    toCurrentQty: 'To Current Qty',
    breakdownConfirmMessage: 'Break down {qty} {fromUnitType} into {result} {toUnitType}?',
    breakdownSuccess: 'Breakdown completed successfully.',
    breakdownError: 'Failed to register breakdown. Please try again later.',
    breakdownQtyExceedsAvailable: 'Breakdown quantity exceeds available stock.',
    conversionRateNotSet:
      'Conversion rate not set in product master. Please contact administrator.',
    // INV-004
    correctionTitle: 'Inventory Correction',
    newQty: 'New Qty',
    diff: 'Difference',
    reason: 'Reason',
    reasonPlaceholder: 'Enter correction reason (max 200 characters)',
    correctionConfirmMessage:
      'Correct {product} ({unitType}) at {loc} from {current} to {newQty}? (Diff: {diff})',
    correctionConfirmZeroMessage: 'New quantity is 0. Inventory will be zero. Continue?',
    correctionSuccess: 'Inventory correction registered.',
    correctionError: 'Failed to register correction. Please try again later.',
    correctionBelowAllocated: 'Cannot correct below allocated quantity ({allocated}).',
    correctionSameQty: 'Same quantity as current stock.',
    correctionReport: 'Correction Report',
    correctionHistory: 'Correction History (Last 5)',
    correctionHistoryDate: 'Date',
    correctionHistoryBefore: 'Before',
    correctionHistoryAfter: 'After',
    correctionHistoryReason: 'Reason',
    correctionHistoryExecutor: 'Performed By',
    correctionHistoryEmpty: 'No correction history.',
    backToList: 'Back to List',
    // INV-011
    stocktakeTitle: 'Stocktake List',
    stocktakeNumber: 'Stocktake ID',
    stocktakeDate: 'Date',
    stocktakeDateFrom: 'Date (From)',
    stocktakeDateTo: 'Date (To)',
    stocktakeBuilding: 'Building',
    stocktakeArea: 'Area',
    stocktakeStatus: 'Status',
    stocktakeStatusAll: 'All',
    stocktakeStatusStarted: 'In Progress',
    stocktakeStatusConfirmed: 'Confirmed',
    stocktakeStaff: 'Staff',
    stocktakeTotalLines: 'Lines',
    stocktakeCountedLines: 'Counted',
    stocktakeProgress: 'Progress',
    stocktakeNew: 'Start Stocktake',
    stocktakeInput: 'Count Entry',
    stocktakeConfirmBtn: 'Confirm',
    stocktakeResult: 'View Result',
    stocktakeView: 'View',
    stocktakeFetchError: 'Failed to fetch stocktake list. Please try again later.',
    stocktakeNoResult: 'No stocktake records matching the criteria.',
    stocktakeDateRangeError: 'Invalid date range.',
    // INV-012
    stocktakeNewTitle: 'Start Stocktake',
    stocktakeLockWarning:
      'Inbound/outbound operations for locations under stocktake will be restricted. Please ensure all operations are completed before starting.',
    stocktakeSelectBuilding: 'Select building',
    stocktakeSelectArea: 'All areas',
    stocktakeTargetLocations: 'Target locations',
    stocktakeTargetDescription: 'Target scope',
    stocktakeNote: 'Note',
    stocktakeNotePlaceholder: 'Enter note (max 200 characters)',
    stocktakeStartConfirm:
      'Start stocktake for {count} locations? Inbound/outbound operations will be restricted after starting.',
    stocktakeStartSuccess: 'Stocktake started (ID: {number})',
    stocktakeStartError: 'Failed to start stocktake. Please try again later.',
    stocktakeConflictError:
      'There is an active stocktake for the target locations. Please confirm it first.',
    stocktakeBuildingRequired: 'Building is required.',
    stocktakeDateRequired: 'Date is required.',
    stocktakePrintList: 'Print Stocktake List',
    // INV-013
    stocktakeDetailTitle: 'Stocktake Count Entry',
    stocktakeBeforeQty: 'Before',
    stocktakeActualQty: 'Actual',
    stocktakeInputStatus: 'Status',
    stocktakeStatusNotCounted: 'N/A',
    stocktakeStatusDiff: 'Diff',
    stocktakeStatusMatch: '✓',
    stocktakeUncountedCount: 'Uncounted: {uncounted} / Total: {total}',
    stocktakeSave: 'Save',
    stocktakeSaveSuccess: 'Saved successfully.',
    stocktakeSaveError: 'Failed to save. Please try again later.',
    stocktakeDetailFetchError: 'Failed to fetch stocktake data.',
    stocktakeGoConfirm: 'Proceed to Confirm',
    stocktakeNotAllCounted:
      'Some items are not counted. Please enter all quantities before confirming.',
    stocktakeCancelConfirm: 'Unsaved changes will be lost. Return to list?',
    stocktakeActualQtyInvalid: 'Actual quantity must be a non-negative integer.',
    // INV-014
    stocktakeConfirmTitle: 'Confirm Stocktake',
    stocktakeDiffSummary: 'Difference Summary',
    stocktakeDiffNone: 'No Diff',
    stocktakeDiffExists: 'Has Diff',
    stocktakeDiffCount: '{count} items',
    stocktakeDiffQty: 'Diff Qty',
    stocktakeDiffRate: 'Diff %',
    stocktakeDiffOnlyFilter: 'Show diff only',
    stocktakeConfirmAction: 'Confirm Stocktake',
    stocktakeConfirmMessage:
      'Confirm stocktake? Inventory will be updated to actual quantities. This cannot be undone.',
    stocktakeConfirmSuccess: 'Stocktake confirmed. Inventory updated (ID: {number})',
    stocktakeConfirmError: 'Failed to confirm stocktake. Please try again later.',
    stocktakeConfirmNotAllCounted: 'Some items are not counted. Please complete count entry first.',
    stocktakeConfirmAlreadyDone: 'This stocktake is already confirmed.',
    stocktakeBackToInput: 'Back to Count Entry',
    stocktakeResultReport: 'Stocktake Result Report',
  },
  returns: {
    title: 'Return Registration',
    breadcrumb: 'Returns',
    breadcrumbNew: 'Return Registration',
    basicInfo: 'Basic Information',
    returnType: 'Return Type',
    returnTypeInbound: 'Inbound Return',
    returnTypeInventory: 'Inventory Return',
    returnTypeOutbound: 'Outbound Return',
    productCode: 'Product Code',
    productName: 'Product Name',
    quantity: 'Quantity',
    unitType: 'Unit Type',
    unitTypeCase: 'Case',
    unitTypeBall: 'Ball',
    unitTypePiece: 'Piece',
    inventorySection: 'Inventory Return Info',
    location: 'Location',
    currentQty: 'Current Stock',
    availableQty: 'Available Stock',
    partnerReasonSection: 'Partner & Reason',
    supplier: 'Supplier',
    supplierName: 'Supplier Name',
    supplierCode: 'Supplier Code',
    supplierPlaceholder: 'Supplier code or name',
    supplierSearchTitle: 'Supplier Search',
    supplierSearchEmpty: 'No suppliers match the search criteria.',
    supplierSearchError: 'Failed to search suppliers.',
    supplierSearchHint: 'Showing 20 of {total} results. Please refine your search.',
    returnReason: 'Return Reason',
    reasonQualityDefect: 'Quality Defect',
    reasonExcessQuantity: 'Excess Quantity',
    reasonWrongDelivery: 'Wrong Delivery',
    reasonExpired: 'Expired',
    reasonDamaged: 'Damaged',
    reasonOther: 'Other',
    returnReasonNote: 'Reason Note',
    conditionalSection: 'Additional Info',
    relatedSlipNumber: 'Related Slip Number',
    lotNumber: 'Lot Number',
    expiryDate: 'Expiry Date',
    register: 'Register',
    confirmMessage:
      'Register return for {productName}: {quantity} {unitType}? (Type: {returnType})',
    registerSuccess: 'Return slip {slipNumber} registered.',
    registerError: 'Failed to register return slip. Please try again later.',
    productNotFound: 'Product code not found.',
    productSearchEmpty: 'No products match the search criteria.',
    productSearchTitle: 'Product Search',
    productCodeColumn: 'Product Code',
    productNameColumn: 'Product Name',
    productSearchHint: 'Showing {count} of {total} results',
    locationCandidateEmpty: 'No locations found with inventory for the selected product/unit type.',
    insufficientQuantity:
      'Return quantity exceeds available stock. (Available: {available} {unitType})',
    allocatedInventory: 'Cannot return allocated inventory. (Allocated: {allocated} {unitType})',
    stocktakeLocked: 'Location is locked for stocktake. Return not allowed.',
    validation: {
      returnTypeRequired: 'Return type is required.',
      productRequired: 'Product code is required.',
      quantityRequired: 'Quantity must be a positive integer.',
      unitTypeRequired: 'Unit type is required.',
      locationRequired: 'Location is required for inventory returns.',
      returnReasonRequired: 'Return reason is required.',
      returnReasonNoteRequired: 'Reason note is required when "Other" is selected.',
      returnReasonNoteMaxLength: 'Reason note must be 500 characters or less.',
    },
  },
  allocation: {
    title: 'Allocation',
    tabExecute: 'Execute Allocation',
    tabAllocated: 'Allocated Orders',
    status: 'Status',
    statusOrdered: 'Ordered',
    statusPartialAllocated: 'Partial Allocated',
    statusAllocated: 'Allocated',
    shippingDateFrom: 'Shipping Date From',
    shippingDateTo: 'Shipping Date To',
    partnerName: 'Ship To',
    slipNumber: 'Order Number',
    plannedDate: 'Shipping Date',
    partner: 'Ship To',
    lineCount: 'Lines',
    orderListTitle: 'Orders for Allocation',
    executeButton: 'Execute Allocation for Selected',
    resultTitle: 'Allocation Result',
    allocatedCount: 'Allocated: {count} orders',
    unpackTitle: 'Unpack Instructions',
    unpackProductCode: 'Product Code',
    unpackProductName: 'Product Name',
    unpackFromUnit: 'From Unit',
    unpackToUnit: 'To Unit',
    unpackQuantity: 'Quantity',
    unpackStatus: 'Status',
    unpackComplete: 'Complete',
    unpackStatusInstructed: 'Instructed',
    unpackStatusCompleted: 'Completed',
    unallocatedTitle: 'Unallocated Lines',
    unallocatedProductCode: 'Product Code',
    unallocatedProductName: 'Product Name',
    unallocatedShortageQty: 'Shortage',
    allocatedListTitle: 'Allocated Orders',
    allocatedLineCount: 'Allocated Lines',
    releaseButton: 'Release Allocation',
    selectRequired: 'Please select at least one order to allocate',
    executeError: 'Allocation failed: {message}',
    releaseNotAllowed: 'Cannot release allocation for orders with picking instructions',
    releaseError: 'Release failed: {message}',
    releaseConfirm:
      'Release allocation for order "{slipNumber}"? Allocated inventory will be restored.',
    executeSuccess: 'Allocation completed ({count} orders)',
    unpackCompleteSuccess: 'Unpack instruction completed',
    releaseSuccess: 'Allocation released',
    unallocatedWarning: 'Some lines could not be allocated due to insufficient inventory',
    fetchError: 'Failed to fetch orders for allocation',
    allocatedFetchError: 'Failed to fetch allocated orders',
    unpackCompleteError: 'Failed to complete unpack instruction',
    alreadyCompleted: 'This unpack instruction is already completed',
  },
  batch: {
    history: {
      title: 'Batch Execution History',
      executedDateFrom: 'Executed Date (From)',
      executedDateTo: 'Executed Date (To)',
      targetBusinessDate: 'Business Date',
      result: 'Result',
      resultAll: 'All',
      resultSuccess: 'Success',
      resultFailed: 'Failed',
      resultRunning: 'Running',
      execId: 'Execution ID',
      executedAt: 'Executed At',
      executedBy: 'Executed By',
      detail: 'Detail',
      drawerTitle: 'Batch Execution Detail',
      stepResults: 'Step Results',
      step1: 'Business Date Update',
      step2: 'Inbound Summary',
      step3: 'Outbound Summary',
      step4: 'Inventory Summary',
      step5: 'Transaction Backup',
      step6: 'Daily Summary Generation',
      stepSuccess: 'Completed',
      stepFailed: 'Failed',
      stepSkipped: 'Skipped (Completed)',
      errorMessage: 'Error Details',
      reportUnreceivedConfirmed: 'Unreceived (Confirmed)',
      reportUnshippedConfirmed: 'Unshipped (Confirmed)',
      reportDialogTitle: 'Report Output',
      reportBusinessDate: 'Business Date',
      reportBusinessDateRequired: 'Business date is required',
      reportOutput: 'PDF Output',
      reportUnreceivedDownloaded: 'Unreceived list (confirmed) download started.',
      reportUnshippedDownloaded: 'Unshipped list (confirmed) download started.',
      reportUnreceivedNoExecution: 'No daily close execution found for {date}. Please check the business date.',
      reportUnshippedNoExecution: 'No daily close execution found for {date}. Please check the business date.',
      reportDownloadError: 'Failed to generate report. Please try again later.',
      fetchError: 'Failed to fetch batch execution history',
      detailFetchError: 'Failed to fetch batch execution detail',
    },
  },
  interfaceFiles: {
    title: 'File List',
    tabInboundPlan: 'Inbound Plan (IFX-001)',
    tabOrder: 'Order (IFX-002)',
    refresh: 'Refresh',
    fileCount: '{count} file(s)',
    fileName: 'File Name',
    fileSize: 'Size',
    createdAt: 'Created At',
    actions: 'Actions',
    validate: 'Validate',
    import: 'Import',
    historyLink: 'Import History →',
    emptyMessage: 'No files awaiting import.',
    emptyDescription: 'Please wait for CSV files to be placed by the external system.',
    fetchError: 'Failed to fetch file list. Please try again later.',
    validateError: 'Failed to execute validation. Please try again later.',
    importError: 'Import process failed. Please try again later.',
    fileSizeExceeded: 'File size exceeds the limit (50MB).',
    importConfirmTitle: 'Select Import Mode',
    importConfirmMessage: 'Import {fileName}. Please select the import mode.',
    modeSuccessOnly: 'Import success rows only',
    modeDiscard: 'Discard all',
    importSuccessCompleted: 'Import of {fileName} completed. (Success: {success} / Error: {error})',
    importSuccessDiscarded: '{fileName} has been discarded. File moved to processed folder.',
    execute: 'Execute',
  },
  validationResult: {
    title: 'Validation Result — {fileName}',
    summary: 'Summary',
    totalRows: 'Total Rows',
    successCount: 'Success',
    errorCount: 'Errors',
    rows: 'rows',
    errorDetail: 'Error Details',
    rowNumber: 'Row #',
    column: 'Column',
    errorCode: 'Error Code',
    message: 'Message',
    fileError: 'File Error',
    allSuccess: 'All {total} rows passed validation. You can proceed with import.',
    importSuccessOnly: 'Import success rows only',
    importDiscard: 'Discard all',
    confirmSuccessOnly: 'Import {success} validated rows from {fileName}. {error} error rows will be skipped. Proceed?',
    confirmDiscard: 'Discard all data from {fileName}. File will be moved to processed folder. Proceed?',
  },
  interfaceHistory: {
    title: 'Import History',
    filterIfType: 'I/F Type',
    filterIfTypeAll: 'All',
    filterIfTypeInbound: 'Inbound Plan (IFX-001)',
    filterIfTypeOrder: 'Order (IFX-002)',
    filterDateFrom: 'Date From',
    filterDateTo: 'Date To',
    filterResult: 'Result',
    filterResultAll: 'All',
    filterResultCompleted: 'Completed',
    filterResultDiscarded: 'Discarded',
    filterResultFailed: 'Failed',
    filterFileName: 'File Name',
    executedAt: 'Executed At',
    fileName: 'File Name',
    ifType: 'I/F Type',
    totalCount: 'Total',
    successCount: 'Success',
    errorCount: 'Errors',
    mode: 'Mode',
    modeSuccessOnly: 'Success only',
    modeDiscard: 'Discard all',
    status: 'Result',
    statusCompleted: 'Completed',
    statusDiscarded: 'Discarded',
    statusFailed: 'Failed',
    executedBy: 'Executed By',
    ifTypeInbound: 'Inbound Plan',
    ifTypeOrder: 'Order',
    blobMoveWarning: 'Blob move failed',
    emptyMessage: 'No import history found matching the criteria.',
    fetchError: 'Failed to fetch import history. Please try again later.',
  },
  dashboard: {
    welcome: 'Welcome, {name}',
    role: {
      SYSTEM_ADMIN: 'System Administrator',
      WAREHOUSE_MANAGER: 'Warehouse Manager',
      WAREHOUSE_STAFF: 'Warehouse Staff',
      VIEWER: 'Viewer',
    },
  },
  error: {
    network: 'Network error. Please check your connection.',
    server: 'Server error. Please try again later.',
    notFound: 'Data not found',
    conflict: 'Data already exists',
    optimisticLock: 'Data was updated by another user. Please reload.',
    forbidden: 'You do not have permission to perform this action',
    forbiddenDescription:
      'You do not have permission to access this page. Please contact your system administrator if you need access.',
  },
}
