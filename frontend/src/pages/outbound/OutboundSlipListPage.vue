<template>
  <div class="wms-page">
    ...
    <el-card class="search-card">
      <el-form :model="searchForm" inline @submit.prevent="handleSearch">
        <el-form-item :label="t('outbound.slip.slipNumber')">
          <el-input v-model="searchForm.slipNumber" :placeholder="t('outbound.slip.slipNumber')" clearable style="width: 180px" />
        </el-form-item>
        <el-form-item :label="t('outbound.slip.plannedDateFrom')">
          <el-date-picker v-model="searchForm.plannedDateFrom" type="date" value-format="YYYY-MM-DD" style="width: 160px" clearable />
        </el-form-item>
        <el-form-item :label="t('outbound.slip.plannedDateTo')">
          <el-date-picker v-model="searchForm.plannedDateTo" type="date" value-format="YYYY-MM-DD" style="width: 160px" clearable />
        </el-form-item>
        <el-form-item :label="t('outbound.slip.partner')">
          <el-select v-model="searchForm.partnerId" style="width: 200px" clearable filterable :placeholder="t('outbound.slip.partnerAll')">
        ...
        <el-form-item :label="t('outbound.slip.status')">
          <el-select v-model="searchForm.status" style="width: 140px" clearable :placeholder="t('outbound.slip.statusAll')">
            ...
            <el-option :label="t('outbound.slip.statusPartialAllocated')" value="PARTIAL_ALLOCATED" />
            <el-option :label="t('outbound.slip.statusAllocated')" value="ALLOCATED" />
            <el-option :label="t('outbound.slip.statusPickingCompleted')" value="PICKING_COMPLETED" />
    ...
    <el-card class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-header__right">
            <el-button v-if="!isViewer" type="warning" :disabled="selectedIds.length === 0" :loading="loading" @click="handleBulkAllocate">
              {{ t('outbound.slip.bulkAllocate') }}
            </el-button>
            <el-button v-if="!isViewer" type="primary" :icon="Plus" @click="router.push({ name: 'outbound-slip-new' })">
      ...
      <el-table v-loading="loading" :data="items" stripe border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="slipNumber" :label="t('outbound.slip.slipNumber')" width="180">
          <template #default="{ row }">
            <el-link type="primary" @click="router.push({ name: 'outbound-slip-detail', params: { id: row.id } })">
              {{ row.slipNumber }}
            </el-link>
          </template>
        </el-table-column>
        <el-table-column :label="t('outbound.slip.slipType')" width="120" align="center">
          <template #default="{ row }">
            {{ row.slipType === 'NORMAL' ? t('outbound.slip.typeNormal') : t('outbound.slip.typeTransfer') }}
          </template>
        </el-table-column>
...
const {
  items, loading, total, page, pageSize, searchForm, partnerOptions, selectedIds, isViewer,
  fetchList, fetchPartnerOptions, handleSearch, handleReset, handlePageChange, handleSizeChange,
  handleSelectionChange, handleBulkAllocate, handleAllocateSingle,
} = useOutboundSlipList()

function isSelectable(row: { status: string }) {
  return row.status === OutboundSlipStatus.Ordered || row.status === OutboundSlipStatus.PartialAllocated || row.status === OutboundSlipStatus.Allocated
}
...
.table-header__right { display: flex; gap: 8px; }
