    ...
    <template v-if="slip">
      ...
      <el-card>
        ...
        <el-descriptions :column="2" border>
          <el-descriptions-item :label="t('outbound.slip.slipNumber')">
            {{ slip.slipNumber }}
          ...
          <el-descriptions-item :label="t('outbound.slip.plannedDate')">
            {{ slip.plannedDate }}
          </el-descriptions-item>
          <el-descriptions-item v-if="slip.transferSlipNumber" label="振替伝票番号">
            {{ slip.transferSlipNumber }}
          </el-descriptions-item>
          <el-descriptions-item :label="t('outbound.slip.partner')">
            <span v-if="slip.partnerCode">{{ slip.partnerCode }} — {{ slip.partnerName }}</span>
            <span v-else>—</span>
          </el-descriptions-item>
          <el-descriptions-item :label="t('outbound.slip.note')" :span="2">
            {{ slip.note ?? '—' }}
          </el-descriptions-item>
          <el-descriptions-item :label="t('outbound.slip.createdAt')">
            {{ formatDateTime(slip.createdAt) }}
          </el-descriptions-item>
          <el-descriptions-item :label="t('outbound.slip.createdBy')">
            {{ slip.createdBy }}
          </el-descriptions-item>
          <el-descriptions-item v-if="slip.shippedAt" :label="t('outbound.slip.shippedAt')">
            {{ formatDateTime(slip.shippedAt) }}
          </el-descriptions-item>
          <el-descriptions-item v-if="slip.carrier" :label="t('outbound.slip.carrier')">
            {{ slip.carrier }}
          </el-descriptions-item>
          <el-descriptions-item
            v-if="slip.trackingNumber"
            :label="t('outbound.slip.trackingNumber')"
          >
            {{ slip.trackingNumber }}
          </el-descriptions-item>
          <el-descriptions-item v-if="slip.cancelledAt" :label="t('outbound.slip.cancelledAt')">
            {{ formatDateTime(slip.cancelledAt) }}
          </el-descriptions-item>
        </el-descriptions>
      </el-card>
      ...
      <el-card>
        ...
        <el-table :data="slip.lines" stripe border style="width: 100%">
          ...
          <el-table-column
            prop="productName"
            :label="t('outbound.slip.productName')"
            min-width="160"
          />
          <el-table-column
            prop="unitType"
            :label="t('outbound.slip.unitType')"
            width="80"
            align="center"
          />
          <el-table-column
            prop="orderedQty"
            :label="t('outbound.slip.orderedQty')"
            width="90"
            align="right"
          />
          <el-table-column :label="t('outbound.slip.allocatedQty')" width="90" align="right">
            <template #default="{ row }">
              {{ row.allocatedQty != null ? row.allocatedQty : '—' }}
            </template>
          </el-table-column>
          <el-table-column :label="t('outbound.slip.pickingQty')" width="110" align="right">
            <template #default="{ row }">
              {{ row.pickingQty != null ? row.pickingQty : '—' }}
            </template>
          </el-table-column>
