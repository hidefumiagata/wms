...
# Test Execution
- **テスト実行コマンドは並列実行禁止**。`./gradlew test`, `npm test`, `vitest`, `pytest` などのテスト系コマンドを複数同時に走らせない（バックエンドとフロントエンドの同時実行も不可）。理由: PC リソースが耐えられず固まる。
- 複数モジュール／複数ファイルのテストが必要な場合も、必ず逐次実行する。