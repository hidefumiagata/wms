...
5. ブランチをpushしてPRを作成する（mainブランチへ向けて）
   - feature / bugfix ラベルのPRでは、PR本文にテストカバレッジを記載する（以下の形式）
   ```
   ## Test coverage
   | 指標 | 値 |
   |------|-----|
   ...
   | C1（ブランチ） | XX% |
   ```
   - `./gradlew test jacocoTestReport` でカバレッジを計測し、実測値を記載する
   - C2（条件網羅）はJaCoCoで計測不可のため記載対象外
6. PRに対して以下のレビューを実施し、結果をPRコメントに追記する（PRラベルにより実施するレビューが異なる）
   - **専門家レビュー**: 対象領域の技術的品質をレビューする（コード品質・パフォーマンス等）
   - **セキュリティレビュー**: セキュリティ専門家の観点でSAST的な静的解析を行う。OWASP Top 10、インジェクション、認証・認可の不備、機密情報の漏洩、依存ライブラリの脆弱性等をチェックする
   - **ドキュメント整合レビュー**（docsラベルのPRのみ）: `docs/` 配下の全ドキュメントを読み込み、変更箇所と他ドキュメント間のクロスチェックを行う。矛盾、修正漏れ、SSOT違反がないかを確認する
   - **設計準拠レビュー**（feature / bugfixラベルのPRのみ）: `docs/architecture-design/` および `docs/test-specifications/` 配下の全ファイルをコンテキストに入れたサブエージェントが、PRの変更差分が設計書の技術方針・規約に準拠しているかをチェックする。チェック観点の例:
     ...
     - トランザクション管理方針
     - ロギング規約
7. レビュー指摘事項は可能な限り対応する（1件ごとに変更対応→コミット→Push→PRコメント追記）
   - **Issueを作成するのは以下の場合のみ**:
     - 設計書（docs/配下）の修正が必要な場合
     - 設計判断・方針決定が必要な場合
   - 上記以外のコード修正・テスト追加・リファクタリング等は、そのPR内で対応を完了させる
