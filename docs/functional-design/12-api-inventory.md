...
在庫管理の中核テーブル。ロケーション・商品・荷姿・ロット番号・賞味期限の5軸で在庫を管理する。

| カラム名 | 型 | NULL | 説明 |
|---------|-----|------|------|
| `id` | bigserial | NOT NULL | PK |
| `warehouse_id` | bigint | NOT NULL | FK → warehouses.id（冗長項目。検索高速化のため非正規化） |
| `location_id` | bigint | NOT NULL | FK → locations.id |
| `product_id` | bigint | NOT NULL | FK → products.id |
| `unit_type` | varchar(10) | NOT NULL | 荷姿: `CASE` / `BALL` / `PIECE` |
| `lot_number` | varchar(100) | NULL | ロット番号（ロット管理商品のみ） |
| `expiry_date` | date | NULL | 賞味/使用期限（期限管理商品のみ） |
| `quantity` | int | NOT NULL | 在庫数量（0以上） |
| `version` | bigint | NOT NULL | 楽観的ロック用バージョン番号（`@Version`） |
| `updated_at` | timestamptz | NOT NULL | 更新日時 |

**制約**:
- `UNIQUE (location_id, product_id, unit_type, lot_number, expiry_date) NULLS NOT DISTINCT`
- `CHECK (quantity >= 0)`

### inventory_movements（在庫変動履歴）

在庫の増減が発生するすべての操作を記録する追記専用テーブル。
...
| `expiry_date` | date | NULL | 賞味/使用期限 |
| `movement_type` | varchar(30) | NOT NULL | 変動種別（下記参照） |
| `quantity` | int | NOT NULL | 変動数量（正=入庫方向、負=出庫方向） |
| `quantity_after` | int | NOT NULL | 変動後在庫数 |
| `reference_id` | bigint | NULL | 参照元レコードID |
| `reference_type` | varchar(50) | NULL | 参照元テーブル種別（例: `INBOUND_LINE`, `STOCKTAKE_HEADER`） |
| `correction_reason` | varchar(500) | NULL | 訂正理由（movement_type=CORRECTIONの場合のみ） |
| `executed_at` | timestamptz | NOT NULL | 変動発生日時 |
| `executed_by` | bigint | NOT NULL | FK → users.id（実行者） |

**movement_type 値一覧**:

| 値 | 説明 | 正負 |
|----|------|------|
| `INBOUND` | 入庫（入荷管理からの入庫） | 正 |
| `INBOUND_CANCEL` | 入庫キャンセル | 負 |
| `OUTBOUND` | 出庫（出荷管理からの出庫） | 負 |
| `MOVE_OUT` | 在庫移動（移動元） | 負 |
| `MOVE_IN` | 在庫移動（移動先） | 正 |
| `BREAKDOWN_OUT` | ばらし（ばらし元） | 負 |
| `BREAKDOWN_IN` | ばらし（ばらし先） | 正 |
| `CORRECTION` | 在庫訂正 | 正または負 |
| `STOCKTAKE_ADJUSTMENT` | 棚卸差異調整 | 正または負 |
...
|---------|-----|------|------|
| `id` | bigserial | NOT NULL | PK |
| `stocktake_number` | varchar(50) | NOT NULL | 棚卸番号（自動採番: `ST-YYYY-NNNNN` 形式） |
| `warehouse_id` | bigint | NOT NULL | FK → warehouses.id |
| `target_description` | varchar(500) | NULL | 棚卸対象説明（例: "A棟 全エリア", "B棟 冷蔵エリア"） |
| `status` | varchar(20) | NOT NULL | ステータス: `STARTED` / `CONFIRMED` |
| `started_at` | timestamptz | NOT NULL | 棚卸開始日時 |
| `started_by` | bigint | NOT NULL | FK → users.id（棚卸開始者） |
| `confirmed_at` | timestamptz | NULL | 棚卸確定日時 |
| `confirmed_by` | bigint | NULL | FK → users.id（棚卸確定者） |
...
| `location_id` | bigint | NOT NULL | FK → locations.id |
| `location_code` | varchar(50) | NOT NULL | ロケーションコード（棚卸開始時コピー） |
| `product_id` | bigint | NOT NULL | FK → products.id |
| `product_code` | varchar(50) | NOT NULL | 商品コード（棚卸開始時コピー） |
| `product_name` | varchar(200) | NOT NULL | 商品名（棚卸開始時コピー） |
| `unit_type` | varchar(10) | NOT NULL | 荷姿 |
| `lot_number` | varchar(100) | NULL | ロット番号 |
| `expiry_date` | date | NULL | 賞味/使用期限 |
| `quantity_before` | int | NOT NULL | 棚卸開始時の在庫数 |
| `quantity_counted` | int | NULL | 実数（担当者入力値） |
| `quantity_diff` | int | NULL | 差異数（`quantity_counted - quantity_before`。棚卸確定時に算出） |
| `is_counted` | boolean | NOT NULL | `false` | 実数入力済みフラグ |
| `counted_at` | timestamptz | NULL | 実数入力日時 |
| `counted_by` | bigint | NULL | FK → users.id（実数入力者） |

**制約**:
- `UNIQUE (stocktake_header_id, location_id, product_id, unit_type, lot_number, expiry_date) NULLS NOT DISTINCT`
...
flowchart TD
    ...
    LOCK_FROM -->|OK| CHECK_QTY{移動元在庫数 >= moveQty?}
    ...
    CHECK_CAP -->|OK| UPDATE_FROM[移動元: quantity -= moveQty]
...
| 4 | 移動元の在庫数 ≥ `moveQty` であること | `INVENTORY_INSUFFICIENT` |
...
flowchart TD
    ...
    LOCK_FROM -->|存在しない| ERR_INV[404 INVENTORY_NOT_FOUND]
    LOCK_FROM -->|OK| CHECK_QTY{ばらし元在庫数 >= breakdownQty?}
    CHECK_QTY -->|不足| ERR_INSUF[422 INVENTORY_INSUFFICIENT]
...
| 4 | ばらし元に対象在庫が存在し、在庫数 ≥ `breakdownQty` であること | `INVENTORY_INSUFFICIENT` |
...
flowchart TD
    ...
    LOCK_INV -->|OK| SAVE_BEFORE[訂正前数量を記録\nquantityBefore = 現在のquantity]
...
- 在庫数がゼロのロケーションについては `stocktake_lines` を作成しない（ロケーション自体にゼロ在庫が存在する場合は作成しない）。
...
stateDiagram-v2
    [*] --> STARTED: 棚卸開始\n(API-INV-012)
    STARTED --> STARTED: 実数入力・一時保存\n(API-INV-014)\n※在庫は変わらない
    STARTED --> CONFIRMED: 棚卸確定\n(API-INV-015)\n全明細is_counted=true が条件\n在庫数を実数で更新\n棚卸ロック解除
    CONFIRMED --> [*]: 確定済み\n（変更不可）
```
...
| `INVENTORY_INSUFFICIENT` | 422 | 在庫数量不足 | INV-002, INV-003 |
