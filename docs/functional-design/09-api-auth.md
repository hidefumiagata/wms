...
| Cookie名 | 説明 | 属性 |
|----------|------|------|
| `access_token` | JWTアクセストークン | `HttpOnly; SameSite=Strict; Path=/; Max-Age=3600` |
| `refresh_token` | リフレッシュトークン（ランダム文字列） | `HttpOnly; SameSite=Strict; Path=/api/v1/auth/refresh; Max-Age=3600` |

```
Set-Cookie: access_token=eyJhbGciOiJIUzI1NiJ9...; HttpOnly; SameSite=Strict; Path=/; Max-Age=3600
Set-Cookie: refresh_token=a1b2c3d4e5f6...; HttpOnly; SameSite=Strict; Path=/api/v1/auth/refresh; Max-Age=3600
```
...
Set-Cookie: refresh_token=; HttpOnly; SameSite=Strict; Path=/api/v1/auth/refresh; Max-Age=0
```
...
```
Set-Cookie: access_token=eyJhbGciOiJIUzI1NiJ9...(新しいJWT); HttpOnly; SameSite=Strict; Path=/; Max-Age=3600
Set-Cookie: refresh_token=f7g8h9i0j1k2...(新しいUUID); HttpOnly; SameSite=Strict; Path=/api/v1/auth/refresh; Max-Age=3600
```
...
| # | ルール | エラーコード |
|---|--------|------------|
| 1 | リフレッシュトークンはスライディング方式を採用する。リフレッシュAPIを呼び出すたびに有効期限が「現在日時 + 1時間」に更新される | — |
...
| `newPassword` | String | ○ | 8文字以上・最大255文字、空白不可 | 新しいパスワード（平文） |
...
| HTTPステータス | エラーコード | 発生条件 |
|-------------|-----------|---------|
| `400 Bad Request` | `VALIDATION_ERROR` | `newPassword` が8文字未満、または必須項目が欠落 |
| `401 Unauthorized` | `UNAUTHORIZED` | `access_token` Cookieが存在しない、または無効なJWT |
...
| 4 | `newPassword` は8文字以上であること（サーバー側でも検証） | `VALIDATION_ERROR` |
...
- **パスワードポリシーの将来拡張**: 現時点では「8文字以上」のみ。将来的に大文字・小文字・数字・記号の組み合わせ要件を追加する可能性がある。
