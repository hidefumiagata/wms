...
- [API-MST-PRD-006 商品コード存在確認](#api-mst-prd-006-商品コード存在確認)
...
| `403 Forbidden` | `FORBIDDEN` | ロールが WAREHOUSE_STAFF または VIEWER |
| `404 Not Found` | `PRODUCT_NOT_FOUND` | 指定IDの商品が存在しない |
| `409 Conflict` | `OPTIMISTIC_LOCK_CONFLICT` | 他のユーザーによって更新済み（`version` 不一致） |
...
| **API名** | 商品無効化/有効化 |
| **メソッド** | `PATCH` |
| **パス** | `/api/v1/master/products/{id}/toggle-active` |
| **認証** | 要 |
| **対象ロール** | SYSTEM_ADMIN, WAREHOUSE_MANAGER |
...
- **パス名について**: エンドポイントのパスは `/toggle-active` だが、`isActive: true` を送信することで有効化も行える。パス名は有効化・無効化の切り替え操作を表す慣用的な命名として許容する。
...
## API-MST-PRD-006 商品コード存在確認

### 1. API概要

| 項目 | 内容 |
|------|------|
| **API ID** | `API-MST-PRD-006` |
| **API名** | 商品コード存在確認 |
| **メソッド** | `GET` |
| **パス** | `/api/v1/master/products/exists` |
| **認証** | 要 |
| **対象ロール** | SYSTEM_ADMIN, WAREHOUSE_MANAGER |
| **概要** | 指定された商品コードがすでに登録されているか確認する。商品登録画面（MST-002）でのリアルタイム重複チェックに使用する。 |
| **関連画面** | MST-002（商品登録） |
...
| パラメータ名 | 型 | 必須 | 説明 |
|------------|-----|:----:|------|
| `productCode` | String | ○ | チェック対象の商品コード |

#### リクエスト例

```
GET /api/v1/master/products/exists?productCode=P-001
```
...
| HTTPステータス | エラーコード | 発生条件 |
|-------------|-----------|--------|
| `400 Bad Request` | `VALIDATION_ERROR` | `productCode` が未指定 |
| `401 Unauthorized` | `UNAUTHORIZED` | 未認証 |
...
```mermaid
flowchart TD
    START([開始: GET /api/v1/master/products/exists]) --> AUTH[JWT認証・ロール確認]
    AUTH -->|未認証| ERR_401[401 UNAUTHORIZED]
    AUTH -->|ロール不足| ERR_403[403 FORBIDDEN]
    AUTH -->|OK| VALIDATE[productCodeパラメータ存在確認]
    VALIDATE -->|未指定| ERR_400[400 VALIDATION_ERROR]
    VALIDATE -->|OK| CHECK["SELECT COUNT(*) FROM products<br/>WHERE product_code = :productCode"]
...
| # | ルール | 詳細 |
|---|--------|------|
| 1 | 無効化済みの商品も存在チェックの対象とする | `is_active` の状態に関わらずコードの一意性をチェックする |
...
- **トランザクション**: 読み取り専用（`@Transactional(readOnly = true)`）。
- **用途**: 商品登録画面（MST-002）で商品コード入力欄のフォーカスアウト時にリアルタイム重複チェックを行うために使用する。登録API（API-MST-PRD-002）でもDB側でユニーク制約チェックを行うため、本APIは UX 向上のための補助的なチェックである。
