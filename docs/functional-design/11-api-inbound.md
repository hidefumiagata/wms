...
stateDiagram-v2
    ...
    PLANNED --> CONFIRMED : 入荷確認（API-INB-confirm）
    PLANNED --> CANCELLED : キャンセル（API-INB-cancel）
    CONFIRMED --> CANCELLED : キャンセル（API-INB-cancel）
    INSPECTING --> CANCELLED : キャンセル（API-INB-cancel）
    PARTIAL_STORED --> CANCELLED : キャンセル（API-INB-cancel）
    CONFIRMED --> INSPECTING : 検品登録（API-INB-006）初回
    INSPECTING --> INSPECTING : 検品登録（API-INB-006）追加
    INSPECTING --> PARTIAL_STORED : 入庫確定（API-INB-007）一部
    INSPECTING --> STORED : 入庫確定（API-INB-007）全件
    PARTIAL_STORED --> PARTIAL_STORED : 入庫確定（API-INB-007）追加
    PARTIAL_STORED --> STORED : 入庫確定（API-INB-007）全件完了
    STORED --> [*]
    CANCELLED --> [*]
```
...
stateDiagram-v2
    [*] --> PENDING : 入荷予定登録時
    PENDING --> INSPECTED : 検品登録（API-INB-006）
    INSPECTED --> INSPECTED : 検品数再登録（上書き可）
    INSPECTED --> STORED : 入庫確定（API-INB-007）
    STORED --> [*]
```
...
| **認証** | 要 |
| **対象ロール** | SYSTEM_ADMIN, WAREHOUSE_MANAGER, WAREHOUSE_STAFF |
| **概要** | ステータスが `PLANNED` の入荷予定伝票のヘッダー情報および明細を更新する。`PLANNED` 以外のステータスでは 409 エラー。 |
| **関連画面** | INB-002（入荷予定登録の編集モード） |
...
API-INB-002 と同形式（`warehouseId` と `slipType` は更新不可のため無視する）。
...
|------------|-----|:----:|-------------|------|
| `partnerId` | Long | 条件必須 | NORMAL入荷の場合必須 | 仕入先ID |
| `plannedDate` | String | ○ | 必須・現在営業日以降 | 入荷予定日 |
| `note` | String | — | 最大500文字 | 備考 |
| `lines` | Array | ○ | 1件以上 | 入荷明細リスト（全明細を差し替え） |
| `lines[].productId` | Long | ○ | API-INB-002 と同様 | 商品ID |
| `lines[].unitType` | String | ○ | `CASE` / `BALL` / `PIECE` | 荷姿 |
| `lines[].plannedQty` | Integer | ○ | 1以上999999以下 | 入荷予定数量 |
| `lines[].lotNumber` | String | 条件必須 | API-INB-002 と同様 | ロット番号 |
| `lines[].expiryDate` | String | 条件必須 | API-INB-002 と同様 | 賞味/使用期限日 |
...
| `404` | `PARTNER_NOT_FOUND` | 仕入先が存在しない |
| `404` | `PRODUCT_NOT_FOUND` | 商品が存在しない |
| `409` | `INBOUND_INVALID_STATUS` | ステータスが `PLANNED` でない |
| `409` | `DUPLICATE_PRODUCT_IN_LINES` | 明細内の商品ID重複 |
| `422` | `INBOUND_PARTNER_NOT_SUPPLIER` | 仕入先種別不正 |
| `422` | `PLANNED_DATE_TOO_EARLY` | 過去日付 |
| `422` | `LOT_NUMBER_REQUIRED` | ロット番号未指定 |
| `422` | `EXPIRY_DATE_REQUIRED` | 期限日未指定 |
...
flowchart TD
    ...
    CHECK_STATUS -->|YES| VALIDATE[入力バリデーション\n・API-INB-002と同様のチェック]
    VALIDATE -->|NG| ERR_VAL[400/404/422 各エラー]
    VALIDATE -->|OK| UPD_HEADER[inbound_slips UPDATE\nplannedDate, partnerId, note 等]
    UPD_HEADER --> DEL_LINES[inbound_slip_lines DELETE\nWHERE inbound_slip_id = ?]
    DEL_LINES --> INS_LINES[inbound_slip_lines INSERT\n明細を全件再登録・line_no採番]
    INS_LINES --> COMMIT[コミット]
...
| # | ルール | エラーコード |
|---|--------|------------|
| 1 | ステータスが `PLANNED` の伝票のみ更新可能 | `INBOUND_INVALID_STATUS` |
| 2 | 明細は全件差し替え方式（既存明細を全DELETE後、リクエストの明細を全INSERT） | — |
| 3 | `warehouseId` と `slipType` は変更不可（リクエストに含まれても無視） | — |

---

### 5. 補足事項

- 楽観的ロックは適用しない（`PLANNED` ステータスでの競合は業務上稀なため）。
- 明細の全DELETE+INSERTにより、`line_no` は 1 から連番で採番し直す。
