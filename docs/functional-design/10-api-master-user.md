...
flowchart TD
    ...
    CHK_SELF_DEACTIVATE -->|No| UPDATE["UPDATE users SET\n- full_name\n- email\n- role\n- is_active\n- updated_by = ログイン中ユーザーID\n- updated_at = NOW()\nWHERE id = :id"]
...
- トランザクション: UPDATE は単一トランザクションで処理する。楽観的ロックは行わない（管理者操作のため）。
...
flowchart TD
    ...
    CHK_SELF -->|No| UPDATE["UPDATE users SET\n- is_active = :isActive\n- updated_by = ログイン中ユーザーID\n- updated_at = NOW()\nWHERE id = :id"]
...
flowchart TD
    ...
    CHK_LOCKED -->|locked=true| UNLOCK["UPDATE users SET\n- locked = false\n- failed_login_count = 0\n- locked_at = null\n- updated_by = ログイン中ユーザーID\n- updated_at = NOW()\nWHERE id = :id"]
...
flowchart TD
    ...
    VALIDATE -->|OK| CHECK["SELECT COUNT(*) FROM users\nWHERE user_code = :code"]
