...
**本ドキュメントの全API（API-MST-FAC-001〜035）はJWT認証必須。** httpOnly Cookie の `access_token` を検証する。各APIの概要テーブルには個別記載しない（共通仕様として本セクションで一括定義）。

認証エラー時は共通エラーレスポンスを返す。
...
> ロール定義は [07-auth-architecture.md](../../architecture-blueprint/07-auth-architecture.md) の認可方式を参照。

### ページネーション（共通）

> ページネーションの共通仕様は [04-backend-architecture.md](../../architecture-blueprint/04-backend-architecture.md) のページネーション共通仕様を参照。

### 共通エラーレスポンス形式

> エラーレスポンスの共通形式は [08-common-infrastructure.md](../../architecture-blueprint/08-common-infrastructure.md) のエラーハンドリングを参照。
...
| API ID | API名 | 関連画面 |
|--------|-------|---------|
| `API-MST-FAC-001` | 倉庫一覧取得 | MST-021 倉庫一覧 |
| `API-MST-FAC-002` | 倉庫登録 | MST-022 倉庫登録 |
| `API-MST-FAC-003` | 倉庫取得 | MST-023 倉庫詳細／編集 |
| `API-MST-FAC-004` | 倉庫更新 | MST-023 倉庫編集 |
| `API-MST-FAC-005` | 倉庫無効化／有効化 | MST-021 倉庫一覧, MST-023 倉庫編集 |
| `API-MST-FAC-006` | 倉庫コード存在確認 | MST-022 倉庫登録 |
| `API-MST-FAC-011` | 棟一覧取得 | MST-031 棟一覧 |
| `API-MST-FAC-012` | 棟登録 | MST-032 棟登録 |
| `API-MST-FAC-013` | 棟取得 | MST-033 棟編集 |
| `API-MST-FAC-014` | 棟更新 | MST-033 棟編集 |
| `API-MST-FAC-015` | 棟無効化／有効化 | MST-031 棟一覧, MST-033 棟編集 |
| `API-MST-FAC-021` | エリア一覧取得 | MST-041 エリア一覧 |
| `API-MST-FAC-022` | エリア登録 | MST-042 エリア登録 |
| `API-MST-FAC-023` | エリア取得 | MST-043 エリア編集 |
| `API-MST-FAC-024` | エリア更新 | MST-043 エリア編集 |
| `API-MST-FAC-025` | エリア無効化／有効化 | MST-041 エリア一覧, MST-043 エリア編集 |
| `API-MST-FAC-031` | ロケーション一覧取得 | MST-051 ロケーション一覧 |
| `API-MST-FAC-032` | ロケーション登録 | MST-052 ロケーション登録 |
| `API-MST-FAC-033` | ロケーション取得 | MST-053 ロケーション編集 |
| `API-MST-FAC-034` | ロケーション更新 | MST-053 ロケーション編集 |
| `API-MST-FAC-035` | ロケーション無効化／有効化 | MST-051 ロケーション一覧, MST-053 ロケーション編集 |
...
| `warehouseNameKana` | string | — | 倉庫名カナ | 最大200文字 |
| `address` | string | — | 住所 | 最大500文字 |
| `version` | Integer | 必須 | 楽観的ロックバージョン（取得時の値を送信） | — |
...
| 403 | `FORBIDDEN` | ロール不足 |
| 404 | `WAREHOUSE_NOT_FOUND` | 指定IDの倉庫が存在しない |
| 409 | `OPTIMISTIC_LOCK_CONFLICT` | 他のユーザーによる更新が先行した（バージョン不一致） |
...
- 楽観的ロックは `version` フィールドで実現する。更新時にリクエストの `version` とDBの `version` を比較し、不一致の場合は `409 OPTIMISTIC_LOCK_CONFLICT` を返す。更新成功時は `version` をインクリメントする。
...
## API-MST-FAC-006 倉庫コード存在確認

### 1. API概要

| 項目 | 内容 |
|------|------|
| **API ID** | `API-MST-FAC-006` |
| **API名** | 倉庫コード存在確認 |
| **メソッド** | `GET` |
...
| パラメータ名 | 型 | 必須 | 説明 |
|------------|-----|:----:|------|
| `warehouseCode` | String | ○ | チェック対象の倉庫コード |

#### リクエスト例

```
GET /api/v1/master/warehouses/exists?warehouseCode=WH-001
```
...
| # | ルール | 詳細 |
|---|--------|------|
| 1 | 無効化済みの倉庫も存在チェックの対象とする | `is_active` の状態に関わらずコードの一意性をチェックする |

### 5. 補足事項

- **トランザクション**: 読み取り専用（`@Transactional(readOnly = true)`）。
- **用途**: 倉庫登録画面（MST-022）で倉庫コード入力欄のフォーカスアウト時にリアルタイム重複チェックを行うために使用する。登録API（API-MST-FAC-002）でもDB側でユニーク制約チェックを行うため、本APIは UX 向上のための補助的なチェックである。
...
flowchart TD
    ...
    F -- Yes --> G{同一倉庫内に buildingCode が重複するか?<br/>UNIQUE warehouse_id, building_code}
...
- **楽観ロック**: `version` フィールドで楽観的ロックを実現する。更新時にリクエストの `version` とDBの `version` を比較し、不一致の場合は `409 OPTIMISTIC_LOCK_CONFLICT` を返す。
...
flowchart TD
    ...
    F -- Yes --> G{同一棟内に areaCode が重複するか?<br/>UNIQUE: building_id, area_code}
    G -- 重複あり --> G1[409 DUPLICATE_CODE]
...
flowchart TD
    ...
    F -- No --> G[warehouseId / codePrefix / areaId / isActive フィルタを組み立て<br/>warehouseId かつ codePrefix がある場合は INDEX: warehouse_id, location_code を使った前方一致を適用]
...
flowchart TD
    ...
    B -- 認証済み --> C{ロールチェック}
    C -- No --> C1[403 FORBIDDEN]
    C -- Yes --> D[リクエストボディのバリデーション<br/>必須チェック・文字数チェック]
    D --> E{バリデーションエラー?}
    E -- Yes --> E1[400 VALIDATION_ERROR]
    E -- No --> F{areaId のエリアが存在するか?}
    F -- No --> F1[404 AREA_NOT_FOUND]
    F -- Yes --> G{エリア種別が STOCK?}
    G -- Yes --> H{locationCode が所定のコードフォーマットに一致するか?}
    H -- No --> H1[400 VALIDATION_ERROR<br/>INVALID_LOCATION_CODE_FORMAT]
    H -- Yes --> I
    G -- No --> I{INBOUND/OUTBOUND/RETURN エリアに<br/>既存ロケーションが存在するか?}
    I -- 既存あり --> I1[422 AREA_LOCATION_LIMIT_EXCEEDED]
    I -- 既存なし --> J
    J{同一倉庫内に locationCode が重複するか?<br/>UNIQUE: warehouse_id, location_code}
...
| `OPTIMISTIC_LOCK_CONFLICT` | 409 | 楽観的ロックのバージョン不一致（他のユーザーによる先行更新） |
