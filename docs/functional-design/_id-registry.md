...
> BAT ID 形式: `BAT-NNN`（3 桁連番形式、デフォルト）または `BAT-{サブカテゴリ}-{名称}`（特定モジュール固有のバッチ。例: `BAT-IF-RECONCILE`）

| BAT ID | バッチ名 | 設計書 | 対応API |
|--------|---------|--------|---------|
| BAT-001 | 日替処理 | BAT-01-daily-close.md | API-BAT-001 |
| BAT-IF-RECONCILE | I/F Blob移動リカバリ | — (Issue #440 で作成予定) | — |

> 注: `設計書` 列の `—` は ID 先行登録のみ完了し個別設計書が対応 Issue で作成される予定であることを示す。`対応 API` 列の `—` は本 ID に対応する API が存在しない（バッチ等）ことを示す。
