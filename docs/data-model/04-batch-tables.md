...
| `step5_status` | varchar(20) | NULL | — | トランデータバックアップ: `SUCCESS` / `FAILED` / `SKIPPED` |
...
| `inbound_quantity_total` | bigint | NOT NULL | — | 入庫数量合計 |
...
| `outbound_quantity_total` | bigint | NOT NULL | — | 出荷数量合計 |
| `created_at` | timestamptz | NOT NULL | now() | 作成日時 |
...
| `total_quantity` | bigint | NOT NULL | — | 在庫数量合計 |
| `created_at` | timestamptz | NOT NULL | now() | 作成日時 |
