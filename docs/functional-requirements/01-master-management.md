...
| 項目 | 内容 |
|------|------|
| **デフォルト倉庫** | ログイン直後は有効な倉庫の中で倉庫コード昇順の先頭を自動選択する |
| **データ反映タイミング** | 画面のデータはボタン操作（検索・登録・更新等）を押すまでサーバーに反映されない |
| **切替時の編集中データ** | 倉庫を切り替えた場合、編集中・入力中のデータは破棄され、新しい倉庫のデータに切り替わる。確認ダイアログは表示しない |
| **切替時の一覧画面** | 倉庫を切り替えると一覧画面は新しい倉庫のデータで自動的に再検索される |
| **Piniaストア管理** | 選択中倉庫IDは `systemStore.selectedWarehouseId` で管理する。全業務画面のAPI呼び出しに倉庫IDを自動付与する |
